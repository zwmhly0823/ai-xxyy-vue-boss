 ...
 * @LastEditTime: 2020-10-16 19:57:45
-->
<template>
  <div class="user-list">
...
export default {
  ...
  methods: {
    ...
    getManagement() {
      ...
      return this.$http.writeApp.User.ManagementForTeacherList(params).then(
        (res) => {
          // console.log(res)
    ...
    getData() {
      ...
      return this.$http.writeApp.User.trialCourseUsersV2(query, page, sort)
        .then((res) => {
          // console.log(res)
    ...
    intentConfirm(data) {
      ...
      this.$http.writeApp.User[urlMap.get(this.curModifyItem.type)](query)
        .then((res) => {
    ...
    intentChange(tar, index) {
      ...
      this.$http.writeApp.User.updateUserInetention(query)
        .then((res) => {
          if (res.code === 0 && res.status === 'OK') {
            const singleData = Object.assign(
              {},
    ...
    getTodayCount(type = 'today') {
      ...
      this.$http.writeApp.User.trialCourseUsersV2(params, 1, {}).then((res) => {
        if (res && res.data && res.data.StudentTrialV2StatisticsPage) {
