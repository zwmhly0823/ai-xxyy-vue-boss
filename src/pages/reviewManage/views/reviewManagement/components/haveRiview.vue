 ...
 * @LastEditTime: 2020-11-02 15:58:13
 */
 -->
<template>
  <div class="container">
    ...
    >
      ...
      <el-table-column label="点评反馈" min-width="200">
        <template slot-scope="scope"
          ><span v-if="scope.row.feedback_type === 0">不满意</span>
          <span v-else-if="scope.row.feedback_type === 1">一般</span>
          <span v-else-if="scope.row.feedback_type === 2">满意</span>
          <span v-else>-</span>
          <br /><span>{{ scope.row.tag }}</span
          ><br /><span>{{ scope.row.content }}</span>
        </template>
      </el-table-column>
    </el-table>
    <m-pagination
