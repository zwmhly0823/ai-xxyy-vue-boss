 ...
 * @LastEditTime: 2020-11-27 18:39:17
-->
<template>
...
export default {
  ...
  methods: {
    ...
    // 根据期获取对应班级
    async getTaskTeamConfigListByTerm() {
      const term = this.currentTerm
      if (!term) return
      this.loading = true
      const {
        payload = []
      } = await this.$http.ReviewManage.getTaskTeamConfigListByTerm(
        term,
        'RESULT'
      )
      console.log(payload)
     ...
     */
    handleCancel() {
      this.$router.push('/realReviewSchedule')
    }
  }
}
</script>
