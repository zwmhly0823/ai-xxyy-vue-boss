 ...
 * @LastEditTime: 2020-12-04 17:57:09
-->
<template>
    ...
    <div class="app-main-container d-flex column">
      ...
      <div class="team-list flex-1">
        ...
        >
          ...
          <el-table-column prop="address" label="是否配置真人点评">
            <template slot-scope="scope">
              <el-switch
...
export default {
  ...
  data() {
    return {
      ...
      termInfo: {},
      // 不需要缓存的path
      noAliveList: ['/realReviewSchedule']
    }
  },
  ...
  created() {
    ...
    if (type === 'edit') {
      ...
      this.getTaskTeamConfigListByTerm(term, false)
    }
  },
    ...
    $route(to, from) {
      ...
      if (this.noAliveList.includes(from.path)) {
        if (this.type === 'add') {
          ...
          this.getTaskTeamConfigListByTerm()
        } else {
          this.getTaskTeamConfigListByTerm(this.currentTerm)
          this.getManagementByPeriod()
        }
      }
    }
  ...
  methods: {
    ...
    async getStage(res) {
      ...
      if (term && this.type === 'add') {
        const { payload } = await this.$http.ReviewManage.taskConfigCheck(term)
        if (payload) {
      ...
      this.getTaskTeamConfigListByTerm(term)
    },
    getDepartment(res) {
    ...
    // 根据期获取对应班级
    async getTaskTeamConfigListByTerm(term, key = true) {
      ...
      const {
        payload = [],
        code
      } = await this.$http.ReviewManage.getTaskTeamConfigListByTerm(term, type)
      if (code === 0) {
        // format data
        this.currentTerm = term
        this.hasSetTerm = false
     ...
     */
    handleCancel() {
      this.$router.replace('/realReviewSchedule')
    },
    ...
    async handleSave() {
      ...
      const { code } = await this.$http.ReviewManage.dispatchTaskTeamConfig(
        data
      )
      loading.close()
      if (code === 0) {
        ...
        setTimeout(() => {
          this.$router.push(`/realReviewSchedule?refresh=1`)
        }, 2000)
      }
    }
  }
