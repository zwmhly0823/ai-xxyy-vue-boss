...
export default {
  ...
  created() {
    console.log('this.classId', this.classId.classId)
  },
  watch: {
    'classId.classId': {
      handler(newval, oldVal) {
        this.tableData = []
        console.log('classId.classId: ' + newval, oldVal) // 但是这两个值打印出来却都是一样的
        this.studentsList()
          .then((res) => {
  ...
  methods: {
    // 学员列表
    async studentsList() {
      const tableResult = await axios.post('/graphql/team', {
        query: `{
      ...
      return new Promise((resolve, reject) => {
        ...
        if (tableResult.data.teamUserListPage) {
          this.totalPages = tableResult.data.teamUserListPage.totalPages * 1
          this.totalElements = +tableResult.data.teamUserListPage.totalElements
          const _data = tableResult.data.teamUserListPage.content
          _data.forEach((ele) => {
