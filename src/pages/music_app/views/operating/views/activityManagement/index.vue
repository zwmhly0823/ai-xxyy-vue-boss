...
<template>
  <el-row type="flex" class="activity-manage app-main">
          ...
          <div class="activity-manage-container">
            ...
            <section ref="tableContainer">
              ...
              >
                ...
                <el-table-column label="下单时间范围" align="center">
                  <template slot-scope="scope">
                    <div v-if="scope.row.promotionsRange === 2">
                      ...
                      @click="changeActivityTime(scope.row,1)"
                    ></i>
                    </div>
                    <div v-else>
                ...
                <el-table-column
                  ...
                  ><template slot-scope="scope">
                    ...
                    <i
                      class="el-icon-edit"
                      style="margin-left:20px;"
                      @click="changeActivityTime(scope.row,2)"
                    ></i>
                  </template>
                </el-table-column>
                  ...
                  <template slot-scope="scope">
                    <div class="editStyle">
                      ...
                      <el-button
                        class="editStyle_btn"
                        type="text"
                        :disabled="isDeleteActivity(scope.row.endDate)"
                        :class="{
                          editStyle_unbtn: isDeleteActivity(scope.row.endDate)
                        }"
    ...
    <el-dialog :title="title" :visible.sync="activityTimeDialog">
      <el-form :model="activityTime">
        <el-form-item label="开始时间" v-if="title == '修改下单时间'">
          <el-date-picker
            v-model="activityTime.startTime"
            type="date"
    ...
    <!-- 删除弹框 -->
    <el-dialog title="删除活动" width="20%" :visible.sync="deleteStatus">
      <span class="el-icon-warning">是否删除活动？</span>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    async deleteConfirm() {
      ...
      let result =  this.$http.liveBroadcast.deleteActivityName(params).then((res) => {
        this.get_PromotionsPageList();
    ...
    activityDetails(row, type) {
      ...
      if(type!=3) {
        ...
        this.promotionsId = row.id
      }
    },
    // 修改活动时间
    changeActivityTime(row,type) {
      switch(type) {
        case 1:
        this.title = "修改下单时间"
        this.activityTime.startTime = row.orderStartDate
        this.activityTime.endTime = row.orderEndDate
        break
        case 2:
        this.title = "修改活动结束时间"
    ...
    _changeActivityTime() {
      ...
      const params2 = {
        id: this.id,
        orderStartDate:this.activityTime.startTime,
        orderEndDate: this.activityTime.endTime
      }
      let params;
      if(this.title == '修改下单时间') {
        params = params2
      }else {
        params = params1
      }
