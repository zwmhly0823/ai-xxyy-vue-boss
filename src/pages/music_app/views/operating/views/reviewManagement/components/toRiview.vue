...
<template>
  <div class="container">
    <el-table v-loading="loading" element-loading-text="拼命加载中" :data="list">
      <el-table-column label="作品" width="100" align="center">
        <template slot-scope="scope" v-if="scope.row.taskImage">
          <el-image
            class="works-img"
      ...
      <el-table-column label="点评" width="300" align="center">
        <template slot-scope="scope" v-if="scope.row.reviewDataList !== undefined">
          <div class="review-container">
            <div class="top-container">
              <div v-for="(item, index) in Object.keys(scope.row.reviewDataList)" :key="index">
                <div
                  class="title"
                  v-if="
                    scope.row.reviewDataList[item] !== undefined &&
                ...
                >{{ item }}</div>
                <div v-if="item !== '开场白'">
                  <div v-for="(_item, _index) of scope.row.reviewDataList[item]" :key="_index">
                    <div @click="selectNow(scope.$index, item, _index)" class="select-container">
                      <span class="background-round">
                        <span :class="[_item.flag ? 'circle' : '111']"></span>
                      </span>
                      <span>{{ scoreObj[_item.score] }}</span>
            ...
            <div class="bottom-container" v-if="Object.keys(scope.row.reviewDataList).length">
              <el-button type="success" @click="syntheticSpeech(scope.$index, scope.row)">生成</el-button>
              <div class="audio-container">
                <audio v-if="scope.row.showAudio" :src="scope.row.mp3Url" controls ref="audioRef"></audio>
              </div>
              <el-button
                type="primary"
                :disabled="!scope.row.mp3Url"
      ...
      <el-table-column prop="courseName" label="课程" align="center" width="180"></el-table-column>
      <el-table-column label="辅导老师" align="center" width="180">
        <template slot-scope="scope">
          <div>{{ scope.row.teacherRealName }}</div>
        </template>
...
>>>>>>> 1acb8ba5a618c11b6936543e2eadc9a498947df1
