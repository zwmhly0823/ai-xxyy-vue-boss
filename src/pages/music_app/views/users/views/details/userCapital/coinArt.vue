 ...
 * @LastEditTime: 2021-01-15 22:33:06
-->
<template>
...
export default {
  ...
  mounted() {
    ...
    console.log('this.childTab', this.childTab)
  ...
  methods: {
    ...
    reqGetUserCoin() {
      ...
      const { gte: startCtime, lte: endCtime } = this.ctime
      ...
      this.$http.User.gtUserCoin(this.reqParams)
        .then((res = {}) => {
          const { content = [] } = res
          if (content.length) {
            content.forEach((item = {}) => {
              const { transType } = item
              // 任务类型
              ...
              item.ctime = formatDateByType(+item.ctime, '-', 'M')
            })
            this.allDigit = Number(res.totalElements)
    ...
    handleCurrentChange(page) {
      ...
      Object.assign(this.reqParams, { page })
      this.reqGetUserCoin()
    },
    top3Show() {
      // 头3数据取自老爹-总获取
