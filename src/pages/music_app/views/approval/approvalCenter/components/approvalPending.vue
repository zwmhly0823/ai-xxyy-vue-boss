 ...
 * @LastEditTime: 2021-03-24 18:01:39
 -->
<template>
  <div class="container" v-loading="vLoading">
    ...
    <div class="time">
      ...
      <searchPhone name="userTel" @result_lk="getPhone" style="margin-right:20px" />
      <courseTeam @result="getTeamId" style="margin-left:0px;margin-right:20px" />
      <department
        style="margin-right:20px"
        name="DepartmentIds"
        placeholder="全部部门"
      ...
      <group-sell
        ...
        tip="请选择老师"
      />
      <tabTimeSelect style="margin-left:0; margin-right:20px" @result="getSeacherTime" />
      <ActivityName
      ...
      <el-select
        clearable
        size="mini"
        v-model="refundTypeValue"
        placeholder="退款类型筛选"
        @change="refundTypeChange"
      >
        <el-option label="优惠券退款" value="优惠券退款"></el-option>
      ...
      <el-button
        ...
        v-if="flowApprovalIdList && flowApprovalIdList.length"
        @click="batchApproval"
      >批量审核通过</el-button>
    </div>
    <!-- dom -->
    ...
    >
      <el-table-column type="selection" v-if="checkboxChoose"></el-table-column>
      <!-- 该条申请单的申请人(applyId)是登录人-只有自己才能撤销自己的申请单 -->
      <el-table-column width="40">
        <template slot-scope="scope">
          <el-dropdown placement="bottom-start" v-if="scope.row.applyId == staffId">
      ...
      <el-table-column label="申请人部门" width="180">
        <template slot-scope="scope">
          <div>{{ scope.row.applyDepartment || '-' }}</div>
        </template>
      </el-table-column>
      <el-table-column label="审批类型">
        <template slot-scope="scope">
          ...
          <div v-show="scope.row.type === 'ADJUSTMENT_SUP_TRIAL'">体验课调级</div>
          <div v-if="scope.row.type === 'ADJUSTMENT_STAGE'">调期申请</div>
          <div v-else-if="scope.row.type === 'ADJUSTMENT_CLASS'">调班申请</div>
          <div v-if="scope.row.type === 'ADJUSTMENT_SUP'">调级申请</div>
      ...
      <el-table-column label="发起时间" width="180">
        <template slot-scope="scope">
          <div>{{ scope.row.openTime }}</div>
        </template>
      </el-table-column>
      <el-table-column label="当前审批人" prop="approvalName"></el-table-column>
    ...
    >
      <template v-slot:title>
        <h2
          v-if="currentType !== 'UNCREDITED'"
        >{{ drawerApprovalDeatail.addressId ? '补发货审批' : '退款审批' }}</h2>
        <h2 v-else>{{ '无归属订单审批' }}</h2>
      </template>
      <!-- 1-1补发货全套 -->
      <div v-if="drawerApprovalDeatail.addressId" class="approval-replenish">
        ...
        <el-row>
          ...
          <el-col
            :span="20"
            :offset="1"
          >{{ courseLevelReplace(drawerApprovalDeatail.currentLesson) }}</el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          ...
          <el-col :span="20" :offset="1">{{ reasonList[drawerApprovalDeatail.reason] }}</el-col>
        </el-row>
        <el-row>
          <el-col :span="3">发起时间:</el-col>
          <el-col :span="20" :offset="1">{{ drawerApprovalDeatail.ctimeFormdate }}</el-col>
        </el-row>
        <el-row>
          ...
          <el-col
            :span="20"
            :offset="1"
          >{{ drawerApprovalDeatail.status == 'PENDING' ? '待审批' : '-' }}</el-col>
        </el-row>
        <el-row>
        ...
        <el-row class="BOTTOM" v-if="isStaffId">
          <el-col :span="20" :offset="1">
            <el-button type="button" @click="dialogFormVisible_checkbox = true">拒 绝</el-button>
      ...
      <div v-else class="approvallk">
        ...
        <div v-if="currentType !== 'UNCREDITED'">
          ...
          <el-row>
            ...
            <el-col :span="18" :offset="1">
              ...
              >
                ...
                <el-table-column prop="price" label="赠品价格" min-width="120"></el-table-column>
              </el-table>
            </el-col>
          </el-row>
          <el-row
          ...
          <el-row>
            ...
            <el-col :offset="1" :span="22">
              ...
              >
                <el-table-column prop="approvalName" label="发起人/审核人" align="center"></el-table-column>
                <el-table-column prop="statusStr" label="审批状态" align="center"></el-table-column>
                <el-table-column prop="approvalRemark" label="审批意见" align="center"></el-table-column>
                <el-table-column label="操作时间" align="center">
                  <template slot-scope="scope">{{ formatDate(scope.row.utime) }}</template>
                </el-table-column>
              </el-table>
            </el-col>
          </el-row>
        ...
        <div v-if="currentType !== 'UNCREDITED'">
          <el-row class="BOTTOM" v-if="isStaffId">
            <el-col :span="20" :offset="1">
              ...
              <el-button type="button" @click="$refs.wandan.dialogFormVisible = true">新建挽单记录</el-button>
              <el-button type="button" @click="dialogFormVisible_checkbox = true">拒 绝</el-button>
              <el-button type="button" @click="ensureReplenish">同 意</el-button>
            </el-col>
          </el-row>
        ...
        >
          <el-row class="BOTTOM">
            <el-col :span="20" :offset="1">
              <el-button type="button" @click="dialogFormVisible_checkbox = true">拒 绝</el-button>
              <el-button type="primary" @click="ensureReplenish">同 意</el-button>
            </el-col>
          </el-row>
        </div>
      </div>
    ...
    >
      ...
      ></el-input>
      <div v-show="adjustDialogErrShow" class="adjust-dialog-err-p">请输入审批意见</div>
      <div class="adjust-dialog-button-box">
        <el-button
          ...
          "
        >取消</el-button>
        <el-button type="primary" @click="adjustResultDialogConfirm">确定</el-button>
      </div>
    </el-dialog>
    <!-- 数据table里首个展示的三个点用来撤销自己的申请-弹窗 -->
    <el-dialog title="撤销申请" :visible.sync="endback" width="30%" :before-close="closeEndBack">
      <span>你确定要撤销申请嘛！</span>
    ...
    <el-dialog
      width="40%"
      title="修改商品数量"
      :visible.sync="dialogFormVisibleCount"
      :destroy-on-close="true"
    >
      <el-table :data="giftList" width="100%" align="center">
        <el-table-column prop="name" label="商品名称" align="center"></el-table-column>
        <el-table-column align="center" label="数量">
          <template slot-scope="scope">
            <el-input-number :min="1" size="small" v-model="scope.row.count"></el-input-number>
          </template>
        </el-table-column>
      </el-table>
      <div slot="footer">
        <el-button @click="quxiao$count">取 消</el-button>
        <el-button type="primary" @click="confirm$count">确 定</el-button>
      </div>
    </el-dialog>
    <!-- 修改金额弹窗-->
    <el-dialog
    ...
    >
      <el-form :model="form" label-position="top" :rules="rules" ref="refundForm">
        ...
        <el-form-item label="特殊说明" prop="explain">
          <el-input type="textarea" :rows="4" placeholder="请输入修改金额的理由" v-model="form.explain"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      giftList: [], // 补发货支持修改商品数量(diolog——table数据)
      ...
      nodeSort: '', // 退款节点选择
      bind_tag: '', // 退款类型标签
      refundTypeValue: '', // 退款类型
      isRefund: 0, // 选择退款出现0课时
      dialogFormVisible: false, // ↑修改金额表单lk
      dialogFormVisibleCount: false, // ↑修改数量table lk
  ...
  mounted() {
    ...
    this.staffName = staff.staffName // storage体验或系统课老师-realName
    // Parameters:
    this.params = {
      type: '',
      status: 'PENDING',
  ...
  methods: {
    ...
    // 补发货修改商品数量
    async confirm$count() {
      const { code } = await this.$http.Backend.changeCount({
        flowApprovalId: this.drawerApprovalDeatail.flowApprovalId,
        productdetails: this.giftList
      })
        .catch((err) => {
          this.$message({
            message: '商品数量修改失败',
    ...
    getcheckType(val) {
      ...
      if (
        val === 'PROMOTIONS' ||
        (val === 'REFUND' && (this.roleId === '4' || this.roleId === '2'))
      ) {
      ...
      if (val === 'REFUND') {
        ...
        this.isRefund = 0
        this.$delete(this.params, 'isZero')
        this.$delete(this.params, 'approvalUserType')
    ...
    nodeSortChange(val) {
      this.params.page = 1
      this.currentPage = 1
      Object.assign(this.params, { approvalUserType: val })
      if (!val) {
        this.$delete(this.params, 'approvalUserType')
      }
      this.checkPending(this.params)
    ...
    refundTypeChange(val) {
      this.params.page = 1
      this.currentPage = 1
      Object.assign(this.params, { refundTypeStr: val })
      if (!val) {
        this.$delete(this.params, 'refundTypeStr')
      }
      this.checkPending(this.params)
    },
    // 新加手机号
            ...
            if (type === 'ADJUSTMENT_SUP') {
              this.adjustDrawerData.content = this.adjustDrawerData.content.concat(
                [
                  {
                    label: '已上课周期',
                    value: this.courseLevelReplace(payData.currentPeriod) || '-'
                  },
                  {
                    label: '调级级别',
        ...
        .then((res) => {
          ...
          if (res && res.payload) {
            ...
            Object.assign(this.adjustDrawerData, {
              ...
              flowApprovalId: payData.flowApprovalId,
              // 审批权限
              packagePower: !packagePower.includes(
                this.resetParams.staffMobile
              ), // this.resetParams.staffMobile !== '15027913263', // 只有15027913263这个人有权限走随材打包的审批
              content: [
                {
                  label: '申请人',
            ...
            this.$refs.adjustDrawerCom.handleDrawerOpen()
            // 获取商品的版本列表
            this.$refs.adjustDrawerCom.initVersionList(
    ...
    // 批量审批
    batchApproval() {
      // 原有的是赠品批量审批，此次新增退款批量审批
      if (this.type_lk === 'REFUND') {
        this.$confirm('确认选中项全部审批吗?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        })
    ...
    async refundFlows() {
      const params = {
        ids: this.flowApprovalIdList.join(','),
        version: this.version,
        staffId: this.staffId,
        staffName: this.staffName,
        approvalRemark: '批量审核通过',
      ...
      this.vLoading = true
      const resData = await this.refundFlowsRequest(params)
      this.vLoading = false
      if (resData === false) {
        return
      }
    ...
    refundFlowsRequest(params) {
      return this.$http.Approval.refundFlowsRequest(params)
        .then((res) => {
          if (res.status === 'OK') {
            return res.payload
          } else {
    ...
    // adjust-dialog的按钮
    async adjustResultDialogConfirm() {
      if (!this.adjustDrawerData.checkSuggestion) {
        this.adjustDialogErrShow = true
        return
      }
      // 新增功能，先修改版本
      const versionRes = await this.packageBoxProduct()
      if (!versionRes) {
        this.$message.error('修改版本信息失败')
        return
      }
      let isConfirm = null
    ...
    packageBoxProduct() {
      for (let i = 0, len = this.changeVersionList.length; len > i; len--) {
        if (
          !Object.prototype.hasOwnProperty.call(
            this.changeVersionList[len - 1],
            'newProVersion'
          )
        ) {
          this.changeVersionList.splice(len - 1, 1)
        }
      }
      if (!this.changeVersionList.length) {
        return true
      }
      const params = {
        flowApprovalId: this.adjustDrawerData.flowApprovalId - 0,
        productInfoUpdates: this.changeVersionList
      }
      return this.$http.Approval.packageBoxProduct(params)
        .then((res) => {
          if (res.status !== 'OK') {
            return false
          }
